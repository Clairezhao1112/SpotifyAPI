...
search = requests.get("https://api.spotify.com/v1/search",
                      headers=h, params={"q":artist,"type":"artist","limit":1}).json()
if not search["artists"]["items"]:
    exit("Artist not found on Spotify")
aid = search["artists"]["items"][0]["id"]

top = requests.get(f"https://api.spotify.com/v1/artists/{aid}/top-tracks",
                   headers=h, params={"market":"US"}).json()["tracks"]
...
df.to_csv(f"{artist.replace(' ','_')}_spotify_ticketmaster.csv", index=False)
print("Saved", f"{artist.replace(' ','_')}_spotify_ticketmaster.csv")